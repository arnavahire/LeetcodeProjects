public class NimGame {
    ...
    /* NOTE - The first one who got the number that is multiple of 4 (i.e. n % 4 == 0) will lost, otherwise he/she will win.
    Proof:

the base case: when n = 4, as suggested by the hint from the problem, no matter which number that first player, the second player would always be able to pick the remaining number.
...
Following the second case, for numbers between (2*4 = 8) and (3*4=12), which are 9, 10, 11, are winning numbers for the first player again, because the first player can always reduce the number into the death number 8.
    ...
    public static void main(String[] args) {
        ...
        boolean result=ng.canWinNim(numberOfStones);
